...
/// <summary>
/// Fade chuyển cảnh cho Map1 bằng UI Toolkit.
/// Không dùng chữ cinematic.
/// 
/// Cơ chế:
/// - Khi vào Map1: màn hình đen hoàn toàn rồi fade ra cảnh game.
/// - Khi chuyển map: fade từ cảnh game sang màn hình đen rồi mới LoadScene.
/// - Overlay được tạo tự động bằng code, không cần tạo sẵn trong UI Builder.
/// </summary>
public class Map1SceneFadeController : MonoBehaviour
{
    [Header("UI Toolkit")]
    [Tooltip("UIDocument dùng để tạo lớp fade đen phủ toàn màn hình. Kéo Map1PoemDialogueUI hoặc GlobalHUD có UIDocument vào đây.")]
    public UIDocument uiDocument;

    [Header("Fade Settings")]
    [Tooltip("Thời gian fade từ cảnh game sang màn hình đen khi chuyển map.")]
    public float fadeOutDuration = 1f;

    [Tooltip("Thời gian fade từ màn hình đen ra cảnh game khi vào map.")]
    public float fadeInDuration = 1f;

    [Tooltip("Bật để khi scene Map1 mở ra thì tự fade từ đen sang sáng.")]
    ...
    public string fadeOverlayName = "map1-scene-fade-overlay";
    ...
    private void SetupOverlay()
    {
        ...
        if (uiDocument == null)
        {
            Debug.LogError("[Map1SceneFadeController] Chưa có UIDocument.");
            return;
        }
        ...
        if (root == null)
        {
            Debug.LogError("[Map1SceneFadeController] Không tìm thấy rootVisualElement.");
            return;
        }
    ...
    /// <summary>
    /// Gọi hàm này khi muốn fade đen rồi chuyển scene.
    /// </summary>
    public void FadeOutThenLoadScene(string sceneName)
    {
        if (isFading)
        {
            Debug.LogWarning("[Map1SceneFadeController] Đang fade, không gọi lại.");
            return;
        }
    ...
    public IEnumerator FadeInRoutine()
    {
        ...
        {
            float t = fadeInDuration <= 0f ? 1f : timer / fadeInDuration;
        ...
        if (enableDebugLog)
        {
            Debug.Log("[Map1SceneFadeController] Fade In xong.");
        }
    }
    ...
    public IEnumerator FadeOutRoutine()
    {
        ...
        {
            float t = fadeOutDuration <= 0f ? 1f : timer / fadeOutDuration;
            SetOverlayAlpha(t);
        ...
        if (enableDebugLog)
        {
            Debug.Log("[Map1SceneFadeController] Fade Out xong.");
        }
    }
    ...
    private IEnumerator FadeOutThenLoadSceneRoutine(string sceneName)
    {
        ...
        {
            Debug.LogError("[Map1SceneFadeController] Scene Name đang trống.");
            yield break;
        }

        if (enableDebugLog)
        {
            Debug.Log("[Map1SceneFadeController] Load scene: " + sceneName);
        }
