...
public class Map4StoryTrigger : MonoBehaviour
{
    ...
    [Tooltip("Sau khi về Idle, chờ ổn định thêm một chút rồi mới mở thoại.")]
    ...
    void StartWaitingForPlayerIdle(Collider2D playerCollider, Rigidbody2D playerRb, Animator playerAnimator)
    {
        ...
        if (enableDebugLog)
        {
            Debug.Log(gameObject.name + ": Wukong đang nhảy/rơi vào trigger, chờ về Idle rồi mới mở thoại.");
        }
    }

    void WaitUntilPlayerIdleThenTrigger()
    {
    ...
    bool IsPlayerAirborne(Collider2D playerCollider, Rigidbody2D playerRb)
    {
        if (useGroundCheck)
        {
            if (!IsPlayerGrounded(playerCollider))
            {
                return true;
            }
        }
    ...
    bool IsPlayerReadyAfterJump()
    {
        if (waitingPlayerCollider == null) return false;
    ...
    void ActivateTrigger()
    {
        ...
        if (enableDebugLog)
        {
            Debug.Log(gameObject.name + " kích hoạt trigger: " + triggerType);
        }
